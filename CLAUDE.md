...
6. **`createPayment` uses a manual multipart string body — do not switch it back to `FormData`.** On Android New Arch (`newArchEnabled: true`), React Native's native networking layer (both XHR and `fetch` adapters) fails to serialize `FormData._parts` into a valid multipart body, causing `ERR_NETWORK` / `TypeError: Network request failed` before any HTTP response. The fix in `src/features/payments/api/paymentsApi.ts` builds the multipart body as a plain string with an explicit `boundary`, and passes it to `axiosClient.post()` with a manually set `Content-Type: multipart/form-data; boundary=...` header. This works identically on iOS and Android. The request interceptor in `axiosClient.ts` still strips `Content-Type` when body `instanceof FormData` as a safeguard for any future endpoint that might accidentally pass a FormData object.
